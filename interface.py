...
def enviar_mensagem(text_input):
    mensagem = text_input.get()

def main():

    #Insntância da janela
    window = Tk()
    window.title('Inteface Gráfica')
    ...
    button_enviar = Button(window, text = 'Enviar', command = enviar_mensagem(text_input))
